...
import { Search, Plus, Edit2, Trash2, GripVertical, ListOrdered, Table as TableIcon } from "lucide-react";
...
function SortableProductRow({ product, BASE_URL }: any) {
  const {
    attributes,
    listeners,
    setNodeRef,
...
function SortableProductTable({ products, onReorder, BASE_URL }: any) {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    const newList = arrayMove(products, oldIndex, newIndex).map(
    ...
    >
      <div className="overflow-x-auto rounded-lg border border-gray-200">
        <table className="w-full text-sm">
          ...
          <tbody>
            <SortableContext
              items={products.map((p: any) => p._id)}
...
export default function Products() {
  ...
  const handleReorder = async (newList: any[]) => {
    ...
    try {
      await dispatch(
        reorderProducts(
          newList.map((p) => ({ _id: p._id, order: p.order }))
        )
      ).unwrap();
    } catch (err: any) {
  ...
  return (
    <>
      <div className="space-y-6 mx-auto text-foreground">
          ...
          <div className="flex gap-2">
            ...
            >
              {sortMode ? (
                ...
                <>
                  <ListOrdered className="h-4 w-4" /> Reorder Products
                </>
              )}
            </Button>
            {!sortMode && (
              <Link to={`${basePath}/products/add`}>
            ...
            <Card className="shadow-sm border border-border bg-card">
              <CardContent className="space-y-4 p-4">
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 flex-wrap">
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button variant="outline" className="w-full md:w-auto border-border bg-card text-foreground hover:bg-muted">
                        Advanced Filters
                      </Button>
                    </PopoverTrigger>
                    <PopoverContent className="w-full md:w-96 p-4 space-y-4 bg-popover border-border text-popover-foreground">
            ...
            <Card className="shadow-sm border border-border bg-card">
              ...
              <CardContent>
                <div className="overflow-x-auto rounded-lg border border-border">
                  <table className="w-full text-sm table-fixed border-separate border-spacing-0 text-foreground">
                        ...
                        products?.map((product) => {
                          ...
                          return (
                            <React.Fragment key={product._id}>
                              <tr className="hover:bg-table-hover transition-colors text-foreground">
                                <td className="p-3">
                                  <Checkbox
                                ...
                                <td className="p-3 text-right flex gap-2 justify-end items-center">
                                  <Link
                                    ...
                                    className="p-1 text-primary hover:bg-muted rounded-md transition-colors"
                                  >
                                  ...
                                  <ConfirmDialog
                                    ...
                                    danger
                                  >
                                    <Button className="p-1 bg-transparent text-destructive hover:bg-destructive/10 rounded-md transition-colors">
                  ...
                  <div className="flex items-center justify-end gap-2 mt-4">
                    ...
                    {Array.from({ length: totalPages }, (_, i) => (
                      <button
                        key={i}
                        onClick={() => setPage(i + 1)}
                        className={`px-3 py-1 rounded-md text-sm transition-colors ${page === i + 1
