...
export default function CartPage() {
  ...
  const allFlatItems = carts.flatMap((cart) =>
    cart.items.map((item) => ({ ...item, cart }))
  );

  const [itemPage, setItemPage] = useState(1);
  const itemsPerPage = limit;
  const totalItemPages = Math.ceil(allFlatItems.length / itemsPerPage);

  const paginatedItems = allFlatItems.slice(
  ...
  useEffect(() => {
    const fetchData = async () => {
      setTableLoading(true);
      try {
        await dispatch(fetchCartItems({ page, limit: 100, search: debouncedQuery })).unwrap();
      } catch {
        toast.error("Failed to fetch cart items.");
      } finally {
        setTableLoading(false);
  ...
  const grandTotal = allFlatItems.reduce((sum, { quantity, variant_id }) => {
    return sum + quantity * (variant_id?.price || 0);
  }, 0);

  const allItemIds = paginatedItems.map((i) => i._id);
              ...
              <thead className="bg-gray-50 text-gray-700 text-sm font-medium">
                <tr>
                  <th className="p-3 w-10 text-left">
                    <Checkbox
                      checked={allItemIds.length > 0 && selectedIds.length === allItemIds.length}
                      onCheckedChange={(checked) => setSelectedIds(checked ? allItemIds : [])}
                    />
                  </th>
                  <th className="p-3 text-left">User</th>
                  <th className="p-3 text-left">Email</th>
