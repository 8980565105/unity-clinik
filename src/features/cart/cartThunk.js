...
//       const response = await api.post(ROUTES.cart.getAll, identifier, {
//         headers: jsonHeaders(),
//       });
...
import api from "../../services/api";
import { ROUTES } from "../../services/routes";
import { resolveGuestId, getGuestId } from "../../utils/guestId";

const jsonHeaders = () => ({ "Content-Type": "application/json" });

const getCartIdentifier = async () => {
  const user = JSON.parse(localStorage.getItem("user") || "{}");
  const token = localStorage.getItem("token");
  if (token && user?._id) {
    return { user_id: user._id };
  }
  const guest_id = await resolveGuestId();
  return { guest_id };
};
