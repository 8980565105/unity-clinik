...
// // ─── Helper: find cart by user_id OR guest_id ───────────────────────
// const findCart = async (identifier) => {
//   if (identifier.user_id) {
//     return Cart.findOne({ user_id: identifier.user_id });
//   }
...
//       .populate({ path: "items.product_id", select: "name price image images category_id" })
//       .populate("items.variant_id", "price offerprice color size sku image images");
...
//     // Return empty cart shape if not found (not an error)
...
//     if (existing) return sendResponse(res, true, existing, "Cart already exists");

//     const cart = new Cart({ user_id: user_id || null, guest_id: guest_id || null });
...
//   try {
//     const { cart_id, product_id, variant_id, quantity, price, original_price, pack_of } = req.body;
...
//       .populate("items.variant_id", "color size sku price offerprice image images");
...
//     if (!guest_id || !user_id)
//       return sendResponse(res, false, null, "Provide both guest_id and user_id");

//     const guestCart = await Cart.findOne({ guest_id });
//     if (!guestCart || guestCart.items.length === 0) {
//       // Nothing to merge; just return (or create) user cart
...
//     await userCart.save();
//     await Cart.findByIdAndDelete(guestCart._id); // clean up guest cart
