...
const defaultSettings = () => ({
  ...
  prepaid: { freeThreshold: 0, ranges: [] },
  cod: { freeThreshold: 0, ranges: [] },
  weightPrepaid: { freeThreshold: 0, ranges: [] },
  weightCod: { freeThreshold: 0, ranges: [] },
  partialCod: { codType: "fixed", value: 0 },
  general: {
    termService: "",
    privacyPolicy: "",
...
const updateUserSettings = async (req, res) => {
  try {
    ...
    const allowedShippingTypes = ["price", "weight"];
    if (shippingType && !allowedShippingTypes.includes(shippingType)) {
      return sendResponse(res, false, null, "Invalid shippingType");
    }
    ...
    const weightPrepaidErr = validateRanges(
      weightPrepaid?.ranges,
      "Weight Prepaid",
    );
    if (weightPrepaidErr) return sendResponse(res, false, null, weightPrepaidErr);

    const weightCodErr = validateRanges(weightCod?.ranges, "Weight COD");
    if (weightCodErr) return sendResponse(res, false, null, weightCodErr);
    ...
    const updateData = {
      ...
      // price wise
      "prepaid.freeThreshold": Number(prepaid?.freeThreshold) || 0,
      "prepaid.ranges": mapRanges(prepaid?.ranges),
      "cod.freeThreshold": Number(cod?.freeThreshold) || 0,
      "cod.ranges": mapRanges(cod?.ranges),

      // weight wise
      "weightPrepaid.freeThreshold": Number(weightPrepaid?.freeThreshold) || 0,
      "weightPrepaid.ranges": mapRanges(weightPrepaid?.ranges),
      "weightCod.freeThreshold": Number(weightCod?.freeThreshold) || 0,
      "weightCod.ranges": mapRanges(weightCod?.ranges),
...
const getPublicSettings = async (req, res) => {
  try {
    const settings = await SystemSettingModel.findOne({}).select(
      "general shippingType prepaid cod weightPrepaid weightCod partialCod",
    );
    ...
    if (!settings) {
      ...
      return sendResponse(
        ...
        true,
        {
          ...
          prepaid: d.prepaid,
          cod: d.cod,
          weightPrepaid: d.weightPrepaid,
          weightCod: d.weightCod,
          partialCod: d.partialCod,
        },
        "Default public settings",
      );
    }
