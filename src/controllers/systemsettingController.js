...
//     if (!storeId)
//       return sendResponse(res, false, null, "Store ID not found in token");
...
//       "prepaid.ranges": (prepaid?.ranges || []).map((r) => ({
//         from: Number(r.from),
//         to: Number(r.to),
...
// ✅ Helper: storeId get કરો - token અથવા User DB માંથી
const getStoreId = async (req) => {
  // પહેલા token માંથી try કરો
  if (req.user?.storeId) return req.user.storeId;

  // Token માં નથી તો User DB માંથી find કરો
  const userId = req.user?._id || req.user?.id;
  if (!userId) return null;

  const user = await User.findById(userId).select("storeId _id");
  return user?.storeId || user?._id; // storeId હોય તો એ, નહીં તો user._id
};
