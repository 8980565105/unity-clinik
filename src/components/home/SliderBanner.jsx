...
//           style={{ fontSize: s.descSize, opacity: 0.8, marginBottom: "8px" }}
//         >
//           {banner.description}
//         </p>
//         <button
...
//   const tripled = [...bannerData, ...bannerData, ...bannerData];
...
//   useEffect(() => {
//     if (!trackRef.current || cardW === 0 || total === 0) return;
...
//   useEffect(() => {
//     if (isCenter || cardW === 0 || total === 0) return;
//     const timer = setInterval(() => handleNext(), 3000);
//     return () => clearInterval(timer);
//   }, [currentIndex, isCenter, cardW, total]);
...
//     if (isAnimating.current || total === 0 || isCenter || cardW === 0) return;
...
//             >
//               {(isCenter ? bannerData : tripled).map((banner, i) => (
//                 <div
//                   key={`${banner._id}-${i}`}
...
export default function BannerSlider() {
  ...
  // ─── 3+ slides (or 2 slides on mobile): full slider with nav ─────────────

  return (
    <Section>
      <Row>
        {/* Nav buttons — only when slider mode active */}
        <div className="flex items-center justify-end gap-3 mb-4">
          <NavBtn direction="left" onClick={handlePrev} variant="primary" />
          <NavBtn direction="right" onClick={handleNext} variant="primary" />
        </div>
