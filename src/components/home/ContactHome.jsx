...
function ContactHome() {
  ...
  const sectionData = slides?.find((item) => item.section === "contactSection");
  ...
  const shouldShow =
    !sectionData?.showOnPages?.length ||
    sectionData?.showOnPages?.includes(currentPage);

  const contact = sectionData?.contactSection;

  if (!sectionData) return null;
  if (sectionData?.status !== "active") return null;
        ...
        >
          ...
          {contact?.buttonText && (
            <div className="z-10">
              <Button
                variant="common"
                onClick={() => navigate(contact?.buttonLink || "/contact-us")}
                className="rounded-full group flex items-center gap-2"
              >
                {contact.buttonText}
                <ArrowRight
